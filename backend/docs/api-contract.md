...
| `POST /v1/public/articles/:slug/comments` | Header `X-Internal-Key`, `Idempotency-Key`. Body `{ authorName: string(1–80), authorEmail: email (wajib, Q9), body: string(1–2000), parentId?: uuid|null, website?: string }` (`website` = honeypot, harus kosong) | `202 { data: { status: "PENDING" } }` | `400 VALIDATION_FAILED` (mis. `authorEmail` kosong), `404` (artikel tidak terbit, atau `parentId` bukan komentar akar `APPROVED` di artikel yang sama — pesan sama agar status moderasi tidak bocor), `401 INVALID_INTERNAL_KEY`, `429`. Honeypot terisi → **`202` palsu** tanpa menyimpan (A6). Balasan pengunjung diizinkan, nesting maks 1 tingkat. |
